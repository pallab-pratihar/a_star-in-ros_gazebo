...
class VisTree:
    def __init__(self,
        ...
        root = VisTreeNode(heuristic=heuristic,
                   coord=goal_coord,
                   dist_to_goal=0)
        ...
        while len(q)>0:
            ...
            for i in range(n):
                ...
                if (c[0],c[1]) not in open_nodes:
                    # initialize
                    node = VisTreeNode(heuristic=heuristic,
                   coord=(c[0],c[1]),
...
class Astar:
    ...
    def __init__(self,
        ...
        self.heuristic = heuristic  # <-- move this to the top

        # use multi processing to check for obstacles
        self.check_pool = multiprocessing.Pool()

        self.init_coord = State(self.heuristic,
                init_coord,
                init_ori,
                cost_to_come=0.0,
                cost_to_go=self.heuristic(init_coord, goal_coord),
                vt_node=None)

        self.goal_coord = State(self.heuristic,
                goal_coord,
                goal_ori,
                cost_to_come=np.inf,
                cost_to_go=0.0)
        ...
        for rpm_l in rpms:
         for rpm_r in rpms:
            if rpm_l > 0  or rpm_r > 0:
                self.actions.append(Action(self.heuristic,
                                       rpm_left=rpm_l,
                                       rpm_right=rpm_r,
    ...
    def initiate_coord(self,
        ...
        # )
        cog = self.heuristic(coord, self.goal_coord.coord)
        vt_node = None
        new_c = State(self.heuristic,
              coord=coord,
              orientation=ori,
    ...
    def run(self, step=False):
        ...
        while self.goal_reached is False and len(self.open_list) > 0:
            ...
            self.explored_nodes += 1  # Only increment for unique states
    ...
    def backtrack(self, goal_coord : State):
        ...
        """
        # Set the final cost to come for reporting
        self.goal_coord.cost_to_come = goal_coord.cost_to_come
...
if __name__ == "__main__":
    ...
    parser.add_argument('--heuristic', type=str, default='euclidean',
                        choices=['euclidean', 'manhattan', 'diagonal', 'dijkstra'],
    ...
    heuristics = {
        ...
        'dijkstra': heuristic_zero
    }
    selected_heuristic = heuristics[args.heuristic]

    # create map object
    custom_map = Map(heuristic=selected_heuristic, inflate_radius=args.rr)
    ...
    vt = VisTree(heuristic=selected_heuristic,
             corners=corners,
    ...
    print(f"Path cost: {a.goal_coord.cost_to_come:.2f}")
    print(f"Nodes explored: {a.explored_nodes}")
